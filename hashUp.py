...
ms_delta_toDays = lambda msCount: msCount / (1000 * 60 * 60 * 24.0);    # 1000 ms/sec, 60 sec/min, 60min/hr, 24hr/day.
...
defaultHasher = buildHasher();   # Default hasher.
...
def test_quick ():
    ...
    secret = "super_secret";    # It's just another salt.
    signedMsg = h.signWrap(msg, secret);
    assert h.signUnwrap(signedMsg, secret) == msg;
    
    badSignDetected = False;    # Not yet detected.
    try: h.signUnwrap(signedMsg + "--edit", secret);
    except SignatureInvalidError as e:
        badSignDetected = True;
    assert badSignDetected;
    
    badSignDetected = False;    # Again, not yet detected.
    time.sleep(0.01);                                                # Letting time pass between creating and reading signed data.
    try: h.signUnwrap(signedMsg, secret, maxExpiryInDays=0);        # Then checking if sign has expired, expect to be expired.
    except SignatureInvalidError as e:
        badSignDetected = True;
    assert badSignDetected;
