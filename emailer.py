...
IS_SENDING_ENABLED = False; # Disabled by default.          # Disabled => just print to console. Enabled => actaully send via SMTP.
#
...
config = {                                                  # Holds SMTP related configuration settings.
    "host": None,
    "port": None,
    "username": None,
    "password": None,
...
def buildMessage (
        fromName, fromEmail, toList, subject, body,
        subtype, ccList, bccList,
    ):
    "Internal helper for building EmailMessage objects.";
    msg = EmailMessage();
    msg["From"] = "%s <%s>" % (fromName, fromEmail);        # TODO: Consider quoting via email.header.Header(). (Consider q'ing all.)
    msg["To"] = ", ".join(toList);                          # toList is expected to be a list of (raw) email ids. ["a@b.c", "d@e.f" ..]
    if ccList:
        msg["Cc"] = ", ".join(ccList);
    if bccList:
        msg["Bcc"] = ", ".join(bccList);                    # This doesn't work in py2, but works in py3. Tested w/ SMTP broadcast.
    msg["Subject"] = subject;
...
def send (
        toList, subject, body, fromName=None, fromEmail=None, 
        subtype="plain", ccList=None, bccList=None,
    ):
    ...
    bccList = bccList or [];
    if subtype == "plain":
        body = utils.stripEachLine(body);                   # For plain text emails, strip each line to avoid leading whitespace.
    # Build message:
    msg = buildMessage(
        fromName, fromEmail, toList, subject,
        body, subtype, ccList, bccList
    );
    ...
    return msg;     # Returned for insepction or reference.
